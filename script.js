// Fresh Island Foods kiosk dashboard
// Weather location: Belfast, Northern Ireland.
// Change LATITUDE / LONGITUDE / LOCATION_NAME below if required.

const LATITUDE = 53.7796;
const LONGITUDE = -1.387325;
...
function setWeatherAmbience(code, isDay) {
  ...
  else if ([2,3,45,48].includes(code)) weatherClass = "weather-cloudy";
  else if ([51,53,55,56,57,61,63,65,66,67,80,81,82].includes(code)) weatherClass = "weather-rain";
  else if ([71,73,75,77,85,86].includes(code)) weatherClass = "weather-snow";
  else if ([95,96,99].includes(code)) weatherClass = "weather-storm";
...
async function loadWeather() {
  ...
  try {
    ...
    const [condition, icon] = weatherInfo(c.weather_code, c.is_day);
    ...
    document.getElementById("wind").textContent =
      `${windDirection(c.wind_direction_10m)} ${Math.round(c.wind_speed_10m)} km/h`;
    ...
    status.textContent = `Live weather • Updated ${stamp}`;
  } catch (err) {
    console.error(err);
